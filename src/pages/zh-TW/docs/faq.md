...
不必。OmniConsole 可在任何 Windows 11 PC 上執行。桌機、筆電與平板需先使用 [**XFSET**](https://github.com/8bit2qubit/XboxFullScreenExperienceTool) 啟用 Xbox 模式 (FSE)；原生掌機（ROG Xbox Ally X 等）已原生支援。
...
可以 — OmniConsole 支援透過 **Protocol URI**、**執行檔路徑** 或 **封裝套件 （MSIX / APPX）** 新增自訂平台，也可加入封面圖片與啟動參數（僅執行檔路徑類型支援）。組態可匯出與匯入為 JSON。詳見 [自訂平台](/omniconsole-site/zh-TW/docs/custom-platforms)。
